...
serve(async (req) => {
  ...
  try {
    ...
    const { data: speakers, error: fetchError } = await supabaseClient
      .from('speakers')
      .select('id, name, photo_url');

    if (fetchError) {
      throw new Error(`Failed to fetch speakers: ${fetchError.message}`);
    }

    // Normalize function to match filenames to speaker names
    ...
    for (const [filename, file] of Object.entries(zip.files)) {
      ...
      if (!ext || !['png', 'jpg', 'jpeg', 'webp', 'avif', 'gif'].includes(ext)) {
      ...
      const matchingSpeaker = speakers?.find(speaker => 
        normalize(speaker.name) === normalizedFilename
      );
      ...
      const uploadPromise = (async () => {
        try {
          ...
          if (fileContent.length > 10 * 1024 * 1024) {
            console.log(`Compressing large headshot for ${matchingSpeaker.name}`);
            try {
          ...
          const storagePath = `${matchingSpeaker.id}-${Date.now()}.${ext}`;
          ...
          try {
            ...
            const driveResponse = await supabaseClient.functions.invoke('upload-to-google-drive', {
              body: {
                ...
                folderId: 'speaker-headshots',
                speakerName: matchingSpeaker.name,
              },
            });
            ...
            if (driveResponse.error) {
              ...
              console.log(`Uploaded ${filename} to Google Drive for speaker ${matchingSpeaker.name}`);
            }
          } catch (driveError) {
          ...
          results.uploaded.push(`${filename} → ${matchingSpeaker.name}`);
        } catch (error) {
          results.errors.push({
            filename,
